// Snapshot assembly. Deliberately NOT a server-action module.
//
// It used to live behind 'use server', which meant `buildSnapshot` was exported
// as a callable server action — so anyone could invoke it from a browser, and
// each call spends about six provider requests with no auth and no rate limit.
// That is a free way to drain the API quota. The pipeline is the only caller and
// it runs server-side, so the fix is simply not to publish it as an action.
